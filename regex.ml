...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  match regexp with
  | Empty_String ->  let v = fresh() in {q0 = v; qs = [v]; sigma = []; delta = []; fs = [v]}
  | Char c -> let s0 = fresh() in
    let s1 = fresh() in
    {sigma = [c]; qs = [s0;s1]; q0 = s0; fs = [s1]; delta = [(s0, Some c, s1)]}
  | Union (r1, r2) -> let s0 = fresh() in
    let s1 = fresh() in
    let m1 = regexp_to_nfa r1 in
    let m2 = regexp_to_nfa r2 in {q0 = s0; qs = [s0; s1;] @ m1.qs @ m2.qs; fs = [s1]; sigma = m1.sigma @ m2.sigma;
      delta = [(s0, None, m1.q0); (s0, None, m2.q0); (List.hd m1.fs, None, s1); (List.hd m2.fs, None, s1)] @ m1.delta @ m2.delta}
  | Concat (r1, r2) ->
    let m1 = regexp_to_nfa r1 in
    let m2 = regexp_to_nfa r2 in {q0 = m1.q0; qs = m1.qs @ m2.qs; fs = [List.hd m2.fs]; sigma = m1.sigma @ m2.sigma;
      delta = [(List.hd m1.fs, None, m2.q0)] @ m1.delta @ m2.delta}
  | Star r1 -> let m1 = regexp_to_nfa r1 in
  let s0 = fresh() in
    let s1 = fresh() in
    {q0 = s0; qs = [s0;s1] @ m1.qs; fs = [s1]; sigma = m1.sigma;
      delta = [(s0, None, m1.q0); (s0, None, List.hd m1.fs); (s0, None, s1); (s1, None, s0)] @ m1.delta}
